...
class LocalHfEmbeddingClientTests {
    ...
    @Test
    void embedBatchPostsModelInputAndDimensionsToLocalHfBridge() throws Exception {
        AtomicReference<String> requestBody = new AtomicReference<>();
        startServer("{\"embeddings\":[[0.1,0.2],[0.3,0.4]]}", requestBody);

        LocalHfEmbeddingClient client = new LocalHfEmbeddingClient(new OkHttpClient());
        ...
        assertEquals("Qwen/Qwen3-Embedding-0.6B", body.get("model").getAsString());
    ...
    private ModelTarget target() {
        ...
        candidate.setId("qwen-emb-hf-small");
        candidate.setProvider("local-hf");
        candidate.setModel("Qwen/Qwen3-Embedding-0.6B");
    ...
    private void startServer(String response, AtomicReference<String> requestBody) throws IOException {
        ...
        server.createContext("/v1/embeddings", exchange -> handle(exchange, response, requestBody));
        server.start();
    }
